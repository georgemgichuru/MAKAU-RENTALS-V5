...
const TenantSignUpForm = () => {
  ...
  const validateKenyanID = (id) => {
    ...
    const passportPattern = /^[A-Z]{2}\d{7}$/i;
    return nationalIdPattern.test(id) || passportPattern.test(id);
  };
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "emergency_contact":
        if (!value) error = `${name === 'phone_number' ? 'Phone number' : 'Emergency contact'} is required`;
  ...
  // Fetch available units when reaching step 3
  const fetchAvailableUnits = async () => {
    try {
      const units = await apiService.getAvailableUnits();
  ...
  const initiateDepositPayment = async (unitId) => {
    try {
      const response = await apiService.initiateDepositPayment(unitId);
  ...
  const checkPaymentStatus = async (paymentId) => {
    try {
      const response = await apiService.getDepositStatus(paymentId);
      return response;
    } catch (error) {
  ...
  const handlePayment = async (e) => {
    ...
    if (!validateKenyanPhone(formData.phone_number)) {
    ...
    try {
      const response = await initiateDepositPayment(selectedUnit.id);
      ...
      if (response.success) {
        ...
        const pollPaymentStatus = async () => {
          ...
          const poll = setInterval(async () => {
            attempts++;
            const statusResponse = await checkPaymentStatus(response.payment_id);
            ...
            if (statusResponse.status === "Success") {
              ...
              setPaymentStatus(prev => ({
                ...
                mpesa_receipt: statusResponse.mpesa_receipt
              }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Note: File upload would need separate handling with FormData
      // For now, we'll proceed with the basic registration

      toast.success(
        "Registration successful! Your payment has been processed. You can now log in to your account."
      );
  ...
  const validateCurrentStep = () => {
    ...
    if (currentStep === 1) {
      const fields = ['full_name', 'email', 'phone_number', 'government_id', 'emergency_contact'];
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 3) {
      // Fetch available units when moving to payment step
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4">
      <div className="max-w-2xl mx-auto py-8">
        ...
        <div className="bg-white/80 backdrop-blur-sm border rounded-2xl shadow-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none ${
                        validationErrors.government_id ? 'border-red-500' : 'border-gray-300'
                      }`}
                      required
                    />
                    {validationErrors.government_id && (
                      <p className="text-red-500 text-xs mt-1">{validationErrors.government_id}</p>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none ${
                        validationErrors.emergency_contact ? 'border-red-500' : 'border-gray-300'
                      }`}
                      required
                    />
          ...
          {/* Step 4: Unit Selection & Payment */}
          {currentStep === 4 && (
            <div>
              <h2 className="text-xl font-semibold mb-2 flex items-center">
                <FaMobileAlt className="mr-2 text-green-600" />
                Select Unit & Make Payment
              </h2>
              <p className="text-gray-600 mb-6">
                Choose your preferred unit and pay the deposit
              </p>
              ...
              {paymentStatus && (
                <div className={`mb-4 p-4 rounded-lg border flex items-start ${
                  ...
                  <div>
                    ...
                    {paymentStatus.checkout_request_id && (
                      <p className="text-sm mt-1">Request ID: {paymentStatus.checkout_request_id}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handlePayment} className="space-y-6">
                {/* Unit Selection */}
                <div>
                  <label className="block text-sm font-medium mb-2">Select Available Unit *</label>
                      ...
                      {availableUnits.map((unit) => (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="font-medium text-lg">{unit.property_name} - Unit {unit.unit_number}</h3>
                              <p className="text-sm text-gray-600">Landlord: {unit.landlord_id}</p>
                              <p className="text-sm text-gray-600">Rent: KSh {unit.rent?.toLocaleString() || 'N/A'}</p>
                              <p className="text-sm text-gray-600">Deposit: KSh {unit.deposit?.toLocaleString() || 'N/A'}</p>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold text-blue-600">
                                Total Due: KSh {((unit.rent || 0) + (unit.deposit || 0)).toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span>KSh {selectedUnit.rent?.toLocaleString() || '0'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Security Deposit:</span>
                        <span>KSh {selectedUnit.deposit?.toLocaleString() || '0'}</span>
                      </div>
                      <div className="flex justify-between font-bold text-lg border-t border-blue-200 pt-2">
                        <span>Total Amount:</span>
                        <span>KSh {((selectedUnit.rent || 0) + (selectedUnit.deposit || 0)).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="submit"
                    disabled={isProcessingPayment || !selectedUnit}
              ...
              <div className="bg-gray-50 rounded-lg p-6 mb-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-gray-600">{formData.full_name}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {selectedUnit ? `${selectedUnit.property_name} - Unit ${selectedUnit.unit_number}` : 'None'}
                    </p>
                  </div>
                  <div>
                    <p className="font-medium">Payment Status:</p>
                    <p className="text-green-600 flex items-center">
                      <FaCheckCircle className="mr-1" /> {paymentStatus?.type === 'success' ? 'Paid' : 'Pending'}
                    </p>
                  </div>
                  <div>
