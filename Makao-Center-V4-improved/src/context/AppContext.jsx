...
const ContextProvider = (props) => {
  ...
  // Fetch tenants from API only when user is logged in
  useEffect(() => {
    if (!isLoggedIn) {
  ...
  // Fetch landlords from API on component mount
  useEffect(() => {
    const fetchLandlords = async () => {
      try {
        ...
        const landlordsData = await apiService.getLandlords();
        console.log('✅ Landlords fetched successfully:', landlordsData);
        setLandlords(landlordsData || []);
      } catch (error) {
        console.error('❌ Failed to fetch landlords:', error);
        setLandlordsError(error.message || 'Failed to load landlords');
        // Keep empty array as fallback
        setLandlords([]);
      } finally {
